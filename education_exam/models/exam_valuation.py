...
class EducationExamValuation(models.Model):
    ...
    division_id = fields.Many2one('standard.division', string='Division',
                                  required=True)
    teachers_id = fields.Many2one('school.teacher', string='Evaluator')
    mark = fields.Integer(string='Max Mark', required=True)
    pass_mark = fields.Integer(string='Pass Mark', required=True)
...
#         return {'domain': {'division_id': domain}}
    ...
    def create_mark_sheet(self):
        ...
        val_id = valuation_line_obj.search([('valuation_id', '=', self.id)])
        if val_id:
            val_id.unlink()
...
class StudentsExamValuationLine(models.Model):
    ...
    mark_scored = fields.Integer(string='Mark')
    pass_or_fail = fields.Boolean(string='Pass/Fail')
    valuation_id = fields.Many2one('education.exam.valuation',
                                   string='Valuation Id')

    reg_no = fields.Char(reslated='student_id.reg_no')
    roll_no = fields.Char(reslated='student_id.roll_no')
    company_id = fields.Many2one(
        'res.company', string='Company',
        default=lambda self: self.env['res.company']._company_default_get())
